...
$configuredSteps = [regex]::Matches($index, '<section[^>]+data-step(?:\s|>)[^>]*data-step-name="([^"]+)"[^>]*>')
Assert-True ($configuredSteps.Count -eq $stepCount -and $configuredSteps[$configuredSteps.Count - 1].Groups[1].Value -eq "Review") "Review must be the final configured wizard step."
...
Assert-True ($index -match 'services-invitation' -and $index -match 'Tell us what needs to work better\. We will turn the goal, workflow, and constraints\s+into a practical first release\.') "The services area must include the outcome-led project-discovery invitation."
...
Assert-True ($index -match 'role="progressbar"' -and $controller -match 'setAttribute\("aria-valuemax", String\(totalSteps\)\)' -and $controller -match 'setAttribute\("aria-valuetext", accessibleProgressText\)') "Progress totals and visible text must share the configured step collection and be exposed to assistive technology."
...
Assert-True ($controller -match 'Math\.min\(Math\.max\(0, currentStep\), finalStepIndex\)' -and $controller -match 'currentStep >= finalStepIndex' -and $controller -match 'nextButton\.hidden = isFinalStep' -and $controller -match 'submitButton\.hidden = !isFinalStep' -and $index -match 'data-form-submit hidden' -and $styles -match '(?s)\[hidden\]\s*\{\s*display:\s*none\s*!important' -and $controller -match 'setMessage\(\);\s*if \(error\?\.name') "Review navigation must stay within the configured steps, show only the final submit action, and clear failed-submission status."
Assert-True ($controller -match 'backButton\.addEventListener\("click"[\s\S]+currentStep = Math\.max\(0, currentStep - 1\)[\s\S]+saveDraftNow\(\)' -and $controller -match 'setTimeout\(saveDraftNow, 350\)' -and $controller -match 'pagehide') "Meaningful changes and navigation must preserve the draft with a short debounce and page-exit flush."
...
Assert-True ($index -match 'data-clear-draft' -and $index -match 'saved on this device' -and $index -match 'Files are not saved with your draft' -and $controller -match 'Your saved answers have been restored\.' -and $controller -match 'intakeDraft\.clear\(window\)') "Customers must be told about device storage and file reselection, see restore feedback, and be able to clear answers."
