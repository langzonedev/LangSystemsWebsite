...
Assert-True ($controller -match 'backButton\.addEventListener\("click"[\s\S]+currentStep = Math\.max\(0, currentStep - 1\)[\s\S]+intakeDraft\.save\(window, form, currentStep\)') "Back must return to the previous configured step while preserving the draft."
